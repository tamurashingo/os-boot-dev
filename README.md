...
詳細なマイルストーンは以下の16ドキュメントにまとめています。1〜67まで全て完了済みです。
...
- [`documents/lisp_console_buffer.md`](documents/lisp_console_buffer.md)（119〜129:
  カーソル制御・画面ダブルバッファリングのロードマップ、119〜129全完了(フェーズI〜L完了)。
  UEFI標準の
  ...
  `os:goto-xy`/`os:print-at`/`os:clear-screen`を追加した。`SetCursorPosition`直接呼び出しの
...
- [`documents/lisp_process_screen_switch.md`](documents/lisp_process_screen_switch.md)
  （130〜138: プロセス毎画面バッファ・OS予約行・Ctrl2回ダイアログ切替のロードマップ、
  130〜138完了。
  画面バッファの先頭行をOSの予約行とし現在アクティブなプロセス名を表示する、画面バッファ
  自体を`LispProcessStack`へ埋め込みプロセス毎に独立させ`lisp_context_switch`で切替時に
  入替・強制全面再描画する、Ctrl2回連続押下をライブに検知して`os:switch-process`相当の
  プロセス選択ダイアログを起動する、という3点を実装する計画。マルチプロセス化ロードマップ
  （`lisp_os_process.md`）のmilestone118で実機検証不能を理由に見送られていた「ConIn/Ex
  protocolのキュー競合リスク」を、`lisp_read_line`自体をEx protocol経由の読み取りへ
  一本化することで構造的に解消する方針を採る。milestone138（ロードマップ完了後の追加要望）で
  Ctrl2回検知を時間ウィンドウ方式から期限のないarmed状態機械へ変更し、状態行左3列を
  システム予約領域として1回目のCtrl押下待機中を示す'C'インジケータを表示、プロセス名の
  表示開始列を4列目（列3）へ変更した。milestone138続報3で、実機での`os:key-debug-log`
  診断結果によりこのファームウェアの`ReadKeyStrokeEx`が修飾キー単体押下のイベント自体を
  生成しないと判明したため、Ctrl2回押下方式自体を廃止し、`ScanCode`が`KeyState`を持たない
  base protocol（ConIn）でも共通に報告されることを利用したF2キー1回押下による即時ダイアログ
  起動方式へ変更した。'C'インジケータおよびarmed待機状態は本変更で無くなった。milestone138
  続報4で、`lisp_screen_buffer_init()`からREPLバナーまでの起動時診断出力が素の`OutputString`
  のままバッファの論理カーソルと同期していなかったためREPLバナーの表示が化ける不具合を修正し、
  当該区間の出力を`console_stream`経由へ統一した。milestone138続報5で、状態行のプロセス名が
  「VM REPL」のように化けて表示される不具合を修正した。画面が一杯になった後は論理カーソルが
  最下行に固定され続けるため、`lisp_screen_flush`が`pending_newlines`分の実`\r\n`を送出する際に
  ハードウェアカーソルが画面最下行付近に残っており、QEMU/OVMFのVT100端末エミュレーションが
  行0を含む全画面のネイティブスクロールを起こして状態行を巻き上げてしまうことが原因だった。
  `\r\n`送出前にハードウェアカーソルを行0直後の安全な行へ一旦退避させるよう修正し、これに伴い
  増えた`SetCursorPosition`呼び出し頻度が既存の非決定的ConOut一時失敗（bug#3系）の再発率を
  上げた副作用は、既存の再試行予算（`LISP_SCREEN_FLUSH_RETRY_MAX`/`_STALL_US`）を4倍に拡大して
  吸収した。milestone138続報5適用後も実機で状態行の化けが解消していないとの指摘を受け、
  QEMUの`-vnc`+生RFBクライアントで実GOPフレームバッファを直接キャプチャする検証手段を新設
  したところ、シリアル出力のVT100シミュレータ再生では検知できていなかった別の根本原因
  （画面最終行・最終列への実書き込みがConOut側の全画面ハードウェアスクロールを誘発し、
  `force_full_redraw`（milestone133）が最終行を描画する度にこれを誘発していた）を特定した
  （milestone138続報6）。`lisp_screen_flush`が最終行の最終列のみハードウェアへの送出範囲から
  除外するよう修正し、実GOPスクリーンショットで状態行の化けが解消したことを確認した
  （リポジトリ直下の`status_row_after_milestone140.png`等を参照）。この過程で状態行とは無関係な
  起動時自己テストログ表示の乱れも発見したが、修正前から存在していたことを確認済みの別問題
  として切り出し、`documents/lisp_process_screen_switch.md`の「既知の未解決事項」に記録した）
