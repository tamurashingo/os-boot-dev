...
- [`documents/lisp_package_system.md`](documents/lisp_package_system.md)（68〜86: パッケージ
  システムをCommonLispサブセットへ再設計するロードマップ、68〜70,72〜81完了・82〜86未着手。
  78で発見したVM/コンパイラの非tail位置let系スタックリークのうち`progn`は78で修正済み、
  `let`/`let*`/`or`側の根本修正は83〜84として追加。81着手時に発見した、`*package*`切替後は
  `common-lisp-user`の特殊形式・ビルトインが無修飾で使えなくなる制約の解消は85〜86として追加）
