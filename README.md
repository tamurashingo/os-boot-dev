...
マーク＆スイープGCまで実装済みで、さらにそれとは独立した実験的なバイトコンパイラ／VMも
動きます。
...
- 起動時に`lisp/stdlib.lisp`（ESPルート直下の`stdlib.lisp`）を自動で`load`し、標準的な
  関数・マクロをLisp自身で定義した状態でREPLが起動する（自己ホスティング）
- スタックマシン型バイトコードVM（`vm-exec`）とLisp自身で書かれたコンパイラ
  （`stdlib.lisp`の`compile-expr`系関数、クロージャ・upvalue・関数呼び出しに対応）。
  通常のREPL評価（`lisp_eval`）とは完全に独立した経路で、`(compile-and-run expr)`のように
  明示的に呼び出したときだけ使われる実験的な実行系
...
詳細なマイルストーンは以下の6ドキュメントにまとめています。1〜46は完了済み、
`lisp_vm_integration.md`（48〜67）は未着手のロードマップです。
...
- `src/main.c` — ブートエントリポイント`EfiMain`（メモリマップ取得・ヒープ初期化・
  自己テスト群・REPLループ）
- `lisp/stdlib.lisp` — 起動時に自動読み込みされる標準ライブラリ（Lisp自身で書かれている）
