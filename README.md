...
Lispファイルを読み込む`load`、および自己ホスティングされた標準ライブラリまで使える状態に
なっています。
...
- `EFI_SIMPLE_FILE_SYSTEM_PROTOCOL`/`EFI_FILE_PROTOCOL`を使い、FAT32のESP上に置いた
  ...
  から使える
- 起動時に`lisp/stdlib.lisp`（ESPルート直下の`stdlib.lisp`）を自動で`load`し、
  `list`/`append`/`reverse`/`mapcar`/`nth`/比較演算子（`<`/`>`/`=`等）などの標準的な
  関数をLisp自身で定義した状態でREPLが起動する
...
[`documents/init_lisp.md`](documents/init_lisp.md)（`defun`〜`load`まで）と
[`documents/bare_metal_lisp.md`](documents/bare_metal_lisp.md)（CommonLisp相当を目指す拡張、
標準ライブラリの自己ホスティングまで）にまとめています。現時点で3ドキュメントすべての
マイルストーンが完了しています。今後の拡張は
[`documents/lisp_robustness.md`](documents/lisp_robustness.md)（自作setjmp/longjmpによる
エラー復旧とマーク＆スイープGCを目指すロードマップ、未着手）に整理しています。
