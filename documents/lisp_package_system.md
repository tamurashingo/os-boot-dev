...
| # | マイルストーン | 状態 | 主な内容 |
|---|---|---|---|
| 68 | `LispPackage`のヒープオブジェクト化 | 未着手 | `LispPackage`を生C構造体から`LISP_TAG_CLOSURE`のescape hatchを共有する新規フィールド群に置き換え、`lisp_alloc_tracked`経由で確保する`lisp_make_package_object`を新設する。symbols/exports/usesは最初からconsリストとして持つ。`lisp_cl_user_package`/`lisp_keyword_package`は生ポインタからタグ付き`LispObject`キャッシュへ変更する。挙動不変な内部置換のみで、`test/lisp/test-package.lisp`が無修正のまま全パスすることを完了条件にする。 |
| 69 | `global_packages`リストと`lisp_make_package`/`lisp_find_package`の一般化 | 未着手 | `global_packages`（consリスト）を新設し`lisp_packages[LISP_MAX_PACKAGES]`/`lisp_package_count`を置き換える。8個上限を撤廃し、現在デッドコードの`lisp_find_package`を実際に`global_packages`を線形探索する実装へ差し替え、名前重複時は既存オブジェクトを返す（冪等性）。この時点ではLisp側APIはまだ公開しない。 |
| 70 | `LispSymbol.package`のタグ付き`LispObject`化 | 未着手 | `LispSymbol.package`を生`LispPackage *`から`LispObject`（#68の新表現へのタグ付きポインタ）へ変更する。`->package->is_keyword_package`を直接読む既存3箇所（eval自己評価分岐/printer/`keywordp`）と`lisp_intern_in_package`を新アクセサ経由に書き換え、`sym->package = 0`（gensym）は`LISP_NIL`に対応させる。`->package`の全参照箇所をgrepで洗い出し漏れなく更新することを完了条件とする。 |
| 71 | パッケージ内シンボル集合のconsリスト化 | 未着手 | `lisp_intern_in_package`の探索・格納先を固定配列`symbols[LISP_MAX_SYMBOLS]`+`symbol_count`からconsリストへ切り替える（探索は線形scanのまま、`LISP_MAX_SYMBOLS`の容量上限とパニックを撤廃）。マイルストーン18の「名前は1回だけ切り詰めてcompare/storeの両方に同じバッファを使う」規律を維持する——本マイルストーンが該当バグを再発させる最大リスク箇所であるため明記する。 |
| 72 | GCルートスキャンの`global_packages`一本化 | 未着手 | `lisp_gc_mark_roots`の「配列を2重ループでシンボルだけ直接mark」を`lisp_gc_mark(global_packages)`1行へ置き換える。`lisp_gc_mark`のclosure分岐に、対象がパッケージ判別済みの場合はsymbols/exports/usesの各consリストも辿る処理を追加する（既存の`vec_data`/`constants`ループと対称）。パッケージ自体・全所属シンボルがGCで回収されないことを保証する新規テスト（多数のシンボルをinternした後`(gc)`を明示実行し`eq`と検索結果が保たれることを確認）を追加する——旧設計にはこの懸念自体が存在しなかったため新規カバレッジ。 |
