...
| # | マイルストーン | 状態 | 主な内容 |
|---|---|---|---|
| 85 | (欠番、`documents/lisp_os_process.md`へ統合) | 統合済み | milestone81の着手時に発見した制約——`*package*`を`common-lisp-user`以外へ切り替えると特殊形式トークン・ビルトインが無修飾で解決できなくなる——の解消は、将来のマルチプロセス化ロードマップ`documents/lisp_os_process.md`のフェーズA・**マイルストーン100**として統合実施する（fork時のプロセス用パッケージがベースパッケージを`use-package`する設計にとって直接の前提条件となるため）。番号85は欠番のまま記録に残す。 |
| 86 | (欠番、`documents/lisp_os_process.md`へ統合) | 統合済み | milestone85修正の回帰検証は、同ロードマップのフェーズA・**マイルストーン101**として統合実施する。番号86は欠番のまま記録に残す。 |
...
は、フェーズA〜Eの実装中に発見した既存バグの根本修正として独立に完了させるため、その完了までを
本ロードマップ全体の完了条件とする。フェーズG（85〜86）はmilestone81の着手時に発見した特殊形式
可視性の問題の切り出しだったが、将来のマルチプロセス化ロードマップ`documents/lisp_os_process.md`
のフェーズA（マイルストーン100・101）へ統合実施することになったため、85・86番自体は欠番のまま
本ロードマップの完了条件からは除外する（実施・検証は統合先のドキュメントで行う）。フェーズH（87）は
フェーズA〜Gと無関係な内容（コンパイラ自身の自己ブートストラップ関数のCスタック深度対策）だが、
ユーザーの判断により本ロードマップの完了条件へ同様に含める。
