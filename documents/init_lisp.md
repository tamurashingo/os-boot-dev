...
| 16 | `load`の実装（FAT32のESPからのファイル読み込み） | ✅ 完了 | `(load "filename")`という組み込み関数を追加した。`src/uefi.h`に`EFI_GUID`/`UINT8`/`UINT16`型と、`EFI_BOOT_SERVICES`に既存の`GetMemoryMap`以降を実UEFI仕様の順序通り（`AllocatePool`/`FreePool`、イベント系6個、`InstallProtocolInterface`系3個をvoid\*プレースホルダとして並べたのち）`HandleProtocol`だけを実型で追加し、新規に`EFI_LOADED_IMAGE_PROTOCOL`（`DeviceHandle`まで）・`EFI_SIMPLE_FILE_SYSTEM_PROTOCOL`（`OpenVolume`）・`EFI_FILE_PROTOCOL`（`Open`/`Close`/`Read`、`Delete`はオフセット保持のためvoid\*のまま）とそれぞれのGUID値を手書きした。`EfiMain`の`ImageHandle`引数は今まで捨てられていたため、`g_system_table`と同じパターンで`g_image_handle`という共有グローバル変数（`lisp.c`で定義、`lisp.h`で`extern`宣言）に保存するよう`main.c`を変更した。`load`本体（`lisp.c`）は、`g_image_handle`を`BootServices->HandleProtocol`に渡して`EFI_LOADED_IMAGE_PROTOCOL`を取得→その`DeviceHandle`から`EFI_SIMPLE_FILE_SYSTEM_PROTOCOL`を取得→`OpenVolume`でルートディレクトリを開く→`Open`で対象ファイルを読み取りモードで開く→`Read`を1回呼んで静的スクラッチバッファ（`lisp_load_buffer`、8KB、CLAUDE.mdの静的バッファ方針を踏襲）に読み込む、という手順を実装した（`GetInfo`によるファイルサイズ問い合わせは行わず、1回のReadで収まる前提とした）。読み込んだバッファは複数のトップレベルS式を含み得るため、`lisp_read`自体は変更せず、新しい`lisp_reader_at_end()`（空白を読み飛ばしてバッファ終端かどうかを判定するだけの小さいヘルパー）を使う`lisp_load_eval_buffer()`を追加し、終端に達するまで`lisp_read`→`lisp_eval`（`global_env`へ）を繰り返す形で「バッファ終端で`lisp_panic`せず読み込み終了とみなす」動作を実現した（単一式読み取り用の`lisp_read`のEOF時panicの挙動はそのまま維持している）。QEMU上で、`esp_dir/test.lsp`に`(defun add10 (x) (+ x 10))`・`(defun double (x) (+ x x))`・`` (defmacro my-when (test body) `(if ,test ,body nil)) ``の3つのトップレベル式を書いたファイルを置き、`(load "test.lsp")`→`my-when`（ファイル内最後の式である`defmacro`の戻り値）、続けて`(add10 5)`→`15`、`(double 7)`→`14`、`(my-when 1 (add10 100))`→`110`、`(my-when nil (add10 100))`→`nil`と、ファイル内で定義した関数・マクロがその後のREPL入力から問題なく使えることを確認した |
