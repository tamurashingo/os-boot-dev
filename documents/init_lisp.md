...
| # | マイルストーン | 状態 | 主な内容 |
|---|---|---|---|
| 12 | グローバル環境の永続化 | 未着手 | `global_env`を`EfiMain`のローカル変数から、REPLの各入力・`defun`・将来の`load`が共有して書き換えられる永続的な状態に変更する。関数呼び出し時の引数↔値バインディング自体はマイルストーン9の`lisp_env_bind_params`をそのまま再利用し、変更しない。`defun`（マイルストーン13）の前提となる |
