...
| # | マイルストーン | 状態 | 主な内容 |
|---|---|---|---|
| 116 | `EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL`導入 | 完了 | `uefi.h`に`EFI_LOCATE_PROTOCOL`(ハンドル指定無しでシステム全体からプロトコルを検索する、既存の`EFI_HANDLE_PROTOCOL`とは別系統のAPI)・`EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL`(`ReadKeyStrokeEx`のみ使用、`Reset`/`WaitForKeyEx`/`SetState`/`RegisterKeyNotify`/`UnregisterKeyNotify`は`void*`のプレースホルダ)・`EFI_KEY_DATA`/`EFI_KEY_STATE`/`EFI_KEY_TOGGLE_STATE`型と、`KeyShiftState`の各ビット定数(`EFI_SHIFT_STATE_VALID`・`EFI_*_CONTROL_PRESSED`等)・GUID値を追加した。**UEFI仕様のバイナリレイアウト制約(CLAUDE.md参照)**: `EFI_BOOT_SERVICES`の既存`HandleProtocol`と新規`LocateProtocol`の間には仕様上20個のフィールド(`Reserved`から`LocateHandleBuffer`まで)が存在し、これらを実際に使わなくても`void*`のプレースホルダとして省略せずに並べる必要があった(オフセットがずれるとvtable経由の呼び出し先が別関数になり検出不能な形で壊れるため)。**設計判断(non-fatalなプロトコル取得)**: 既存の`lisp_open_esp_root`(milestone16)が未検出時に`lisp_panic`するのに対し、新設の`lisp_input_ex_init()`は`LocateProtocol`が失敗しても`g_text_input_ex`を`NULL`のままにして正常に返る(パニックしない)設計にした。理由は、本プロトコルの存在がファームウェア依存であり、無関係な既存28フィクスチャの起動を本マイルストーンの都合で壊すリスクを避けたかったため。**判定ロジックの分離**: 実機/実ファームウェアのキー入力イベント配信に依存しない部分として、`EFI_KEY_DATA`から「Ctrl単体押下」を判定する純粋関数`lisp_key_state_is_lone_ctrl`を`src/lisp.c`に実装した(条件: (1)`EFI_SHIFT_STATE_VALID`が立っている、(2)左右いずれかのCtrlビットが立っている、(3)Shift/Alt/Logo/Menu/SysReqの他修飾ビットが一切立っていない、(4)`Key.ScanCode`/`Key.UnicodeChar`が両方0)。この関数はC自己テスト`lisp_key_state_selftest`(`main.c`の起動シーケンス最初期、`g_system_table`/`g_image_handle`セット直後に実行)から、手作りの`EFI_KEY_DATA`6パターン(左Ctrl単体=true、右Ctrl単体=true、VALIDビット無し=false、Ctrl+Shift同時=false、Ctrl+文字入力=false、修飾無しの通常文字=false)で検証しており、実機キーボード入力には一切依存しない。**確認内容で判明した経験的事実(かつ既知の限界)**: `lisp_input_ex_init()`実行後の診断print("Text Input Ex Protocol: FOUND"/"NOT FOUND")を一時的に`main.c`へ追加して`make test`のヘッドレスQEMU/OVMF環境で観測した結果、`FOUND`が出力され、`LocateProtocol`自体はこのテスト環境でも正常に動作することを確認した(確認後、診断printは削除せず`EfiMainImpl`起動ログの一部として残した)。ただし、**この環境がCtrl単体キー押下イベントを実際に`ReadKeyStrokeEx`経由で配信するかどうかは未検証・未確認のまま残る既知の限界**である。UEFI仕様上、ファームウェア/キーボードドライバが単独修飾キーのイベントをそもそも生成するかは`SetState`による`EFI_KEY_STATE_EXPOSED`相当の設定に依存する可能性があり、`-display none`のヘッドレスQEMUにはそもそも仮想キーボード入力の主体が存在しないため、本マイルストーンでは実際のキー入力による検証は行っていない(本ドキュメントのフェーズH検証方針どおり、実キー入力の検証はGUI経由の個別対話セッションに委ねる)。`make build`/`make test`(28ファイル全PASS、新規自己テスト`Key state (lone Ctrl) self-test: PASS`のログを含む)で既存フィクスチャへの回帰が無いことを確認した。 |
| 117 | Ctrl2回連続判定 | 完了 | `lisp_wait_for_double_ctrl(window_100ns)`(`src/lisp.c`)を実装した。**設計**: `lisp_builtin_sleep`(milestone25)と同じ`CreateEvent`(`EVT_TIMER`)/`SetTimer`(`TimerRelative`)/`WaitForEvent`/`CloseEvent`パターンを、`g_text_input_ex->WaitForKeyEx`との複数イベント同時待ちへ拡張した。まず`WaitForKeyEx`のみを渡した`WaitForEvent`で1回目のCtrl単体押下を無期限に待ち(マッチしない鍵イベントは`ReadKeyStrokeEx`で読み捨てて待ち続ける)、検出したら使い捨てタイマーイベントを`window_100ns`(UEFIネイティブ単位=100ns)でセットし、`[WaitForKeyEx, timer_event]`の2イベントを渡した`WaitForEvent`で2回目のCtrl単体押下がタイマー発火前に来るかを判定する(タイマーイベントは一度`SetTimer`すれば、間に挟まる無関係な鍵イベントで何度`WaitForEvent`を呼び直しても発火予定時刻自体は変わらず消費されずに残るため、ループ内で再セットする必要はない)。`g_text_input_ex`が未検出(`NULL`)なら判定不能として常に偽を返す。**テスト可能性の分離**: 実機のキー入力配信に依存する`WaitForEvent`/`ReadKeyStrokeEx`本体はヘッドレスQEMUでは検証できない(milestone116と同じ既知の限界)ため、「発火した側がタイマーか鍵イベントか(`fired_index`)」と「鍵イベントだった場合その内容」から、タイムアウト/マッチ(Ctrl単体の2回目)/無視して待ち続ける、の3値へ分類する純粋関数`lisp_ctrl_wait_classify`を切り出し、これのみを検証対象とした。C自己テスト`lisp_ctrl_wait_classify_selftest`(`main.c`の起動シーケンスへmilestone116の自己テスト直後に組み込み)を追加し、(1)`fired_index`がタイマー側なら鍵データの内容に関わらずタイムアウトになること、(2)鍵イベント側でCtrl単体押下ならマッチになること、(3)鍵イベント側で通常文字キーなら無視(待ち続ける)になることを確認した。`lisp_wait_for_double_ctrl`自体は118のプロセス切替UIから使われる想定でLispへの公開は行わず、C側の内部インフラのままとした(フェーズC・104-107と同じ方針)。`make build`/`make test`(28ファイル全PASS、`Ctrl double-press classify self-test: PASS`のログを含む)で既存フィクスチャへの回帰が無いことを確認した。 |
| 118 | プロセス一覧UI・REPL切替 | 完了 | `os:switch-process`(`lisp/os.lisp`)を実装した。**スコープの絞り込み(明記)**: 当初の計画はCtrl2回連続押下(117)の検知からの自動発火を想定していたが、`ConIn`(`EFI_SIMPLE_TEXT_INPUT_PROTOCOL`、REPL本体の`lisp_read_line`が使う)と`EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL`(116/117が使う)が実機/ファームウェア上で同一の下位キーストロークキューを共有している可能性があり、その場合メインREPLループのホットパスへライブなCtrl検知を組み込むと、REPL通常入力に向かうべき文字を横取りして静かに入力を破壊するリスクがある。この可能性は`-display none`のヘッドレスQEMU(GUI/実キーボード無し)では検証も反証もできず、`make test`が常時実行する既存28フィクスチャ全てが通るメインREPLループの入力処理にこの未検証のリスクを晒すべきではないと判断し、本マイルストーンはユーザーが明示的に呼び出すコマンド(`os:switch-process`)としてのみ実装した。ライブなCtrl-Ctrl自動発火のメインループへの統合は、実機/GUI環境で検証可能になるまで意図的に見送る(隠さず本行に明記する)。**新規Cビルトイン`%read-console-expr`**: `os:switch-process`のプロセス選択メニューがコンソールから1式読み込むための薄いビルトインを`src/lisp.c`に追加した。プロンプト文字列を表示した後、REPL本体(`main.c`の`EfiMainImpl`)と全く同じ`lisp_read_line`/`lisp_read_from_buffer`(milestone6/8)をそのまま再利用するだけで、`ConIn`以外のプロトコルには一切触れない設計にしたため、`EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL`のキーイベントキューと衝突する余地を新たに持たない(空行が入力された場合はプロンプトを再表示して読み直す)。**Lisp側実装**: `%print-process-entry`/`%print-process-menu`(`os:get-all-processes`が返すリストを`write-string`/`princ`/`write-line`で`"  <index>) <name> [<status>]"`形式の1行ずつ表示する再帰)・`%nth-process-or-nil`(範囲外なら`nil`を返す安全な`nth`)を内部ヘルパーとして追加し、`os:switch-process`本体は一覧表示→`%read-console-expr`で選択番号を読み取る→選択したプロセスの`status`スロットが`nil`(一度もresumeされていない=thunkを持たない)なら`%process-resume`がpanicしてしまうため、その場合は事前に案内メッセージを出して中断する→それ以外は既存の`os:process-resume`(112)をそのまま呼ぶ、という流れにした。`os:process-resume`はブロッキングであり、選択したプロセスが`os:process-suspend`するか実行を終えるまで呼び出し元(このREPL自身)には戻らないため、「呼び出しが戻ってきた時点で制御がこのREPLに戻った」ことがそのまま「アクティブなREPLを選択したプロセスへ切り替える」の実体になっている(フェーズCの104-107・フェーズFの112がそのまま使える設計だったため、118独自の新規コンテキスト切替コードは不要だった)。**遭遇した既存の制約の再発見**: 実装直後の対話検証で、`%print-process-entry`の本体が期待通り出力されず先頭の`(write-string "  ")`だけが実行されて関数がその戻り値`t`を即座に返してしまう不具合に遭遇した。原因は本ドキュメントとは別の既存の恒久的制約(milestone21で確認済みの「`defun`/`lambda`本体は単一form限定、複数の逐次formを書きたい場合は明示的に`(progn ...)`で1つのformへまとめる必要がある」という、compile-and-run経路にも及ぶ挙動)に、複数の`write-string`/`princ`呼び出しを`progn`無しで並べてしまったことで再度抵触したものであり、`%print-process-entry`の本体を`(progn ...)`で包むことで解決した(新規のバグではなく既知の制約への抵触)。**確認内容**: `make build`/`make test`(28ファイル全PASS、回帰無し)に加えて、個別のQEMUシリアル対話セッションで実際に`(os:make-process "demo-1")`→`(os:process-resume ...)`で`os:process-suspend`まで進めて`:suspended`にし、`(os:switch-process)`でプロセス一覧(自身のフィクスチャ用プロセスに加え、起動時自己テストが登録する`PROCESS-0`/`PROCESS-1`も含む)が正しく表示されること、番号`0`を選択すると中断点の直後(`"inside demo-1 after resume"`)から実行が再開され、thunk完了後に`status`が`:finished`になり`"Returned to this REPL."`が出力されて呼び出し元のREPLへ制御が戻ることを確認した。 |
