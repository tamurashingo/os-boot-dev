...
| 53 | lisp_applyのコンパイル済みクロージャ対応（インタプリタ→VM方向） | 未着手 | `lisp_apply`にコンパイル済みクロージャの判定分岐を追加し、`lisp_vm_run`へ委譲するようにする。`mapcar`/`funcall`/`apply`等の既存高階ビルトインと、マクロ呼び出し自体（`lisp_apply`経由）がコンパイル済みクロージャを安全に第一級の値として扱えるようにする。 |
| 54 | compile-expr: progn/let*/cond/and/or/when/unless対応 | 未着手 | 既存の`if`/`let`/ジャンプ命令の組み合わせで表現できる脱糖候補7種を`compile-expr`に追加する。 |
| 55 | compile-expr: block/return-from対応 | 未着手 | `if`/`let`/`lambda`に還元できない非局所脱出のため、タグ付きunwindシグナルを各呼び出しフレームで検査する新規VM機構を追加し、既存インタプリタの`block`/`return-from`実装と対応させる。 |
| 56 | compile-expr: quasiquote対応 | 未着手 | 既存インタプリタのquasiquote展開ロジックと等価なコード生成を`compile-expr`に追加する。 |
| 57 | compile-expr: defvar/defparameter対応 | 未着手 | グローバル動的変数の束縛確立フォームをコンパイル対応する。`defmacro`は対象外とし、恒久的にフォールバックさせる。 |
