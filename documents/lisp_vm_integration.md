...
| 66 | 実stdlib.lispに対するcompile-expr網羅性監査と防御強化 | 完了 | 網羅性監査: milestone65で`not`/`null`/`list`/`append`/`reverse`/`mapcar`が`lisp/compiler.lisp`へ移設された結果、現在の`lisp/stdlib.lisp`（`1+`/`1-`/比較演算子群/`zerop`/`plusp`/`minusp`/`nth`/`reduce`/`sort-insert`/`sort`）が使う特殊形式は`defun`・`if`・`and`の3種のみであることをgrepで確認した（`let`/`let*`/`cond`/`or`/`when`/`unless`/`block`/`return-from`/`defvar`/`defparameter`/quasiquote/`setq`は1件も使用していない）。これらはいずれも`compile-expr`（`lisp/compiler.lisp`）で既にmilestone54/58時点から対応済みであり、追加対応は不要だった。防御強化: 当初のロードマップ記述にあった「1バイトオペランド上限」はmilestone60で既に2byte(0-65535)へ拡張済みであり前提が古くなっていたが、そのオペランドを実際にバイトコード列へ書き込む`vm-make-closure`のCブリッジ(`src/lisp.c`)には、`lisp/compiler.lisp`の`low-byte`/`high-byte`がオペランド値を2byteへ分割する際に0-65535の範囲を超えた場合（`high-byte`が256以上を返す）でもバイト配列への代入時にサイレントに切り捨てて破損したバイトコードのまま実行してしまう欠陥があった。`lisp_builtin_vm_make_closure`のバイトコード読み込みループに、各バイト値が0-255の範囲に収まっているかの明示的なチェックを追加し、外れていれば既存の`lisp_panic_vm_bridge_limit_exceeded`（milestone49で導入済みの診断用panicヘルパー）で`vm-make-closure: bytecode byte out of range (limit 0xff)`という診断可能なメッセージとともに落とすようにした。このチェックは全命令のジャンプ先・定数プール/ローカル/upvalue索引が共通して経由する唯一の書き込み地点であるため、Lisp側の各アセンブラ関数を個別に検証する必要が無い。ブリッジバッファサイズ(`VM_BRIDGE_MAX_BYTECODE`等)は、milestone65で全19フィクスチャがパスしていることから実stdlib.lisp関数のコンパイルに対して現状のサイズ(2048/256/128)のままで十分であることを確認済みとし、変更不要と判断した。テストは`make test`では実施せず（本チェックは意図的にpanic/ハングするため）、`(vm-make-closure 0 (list 256) nil nil)`を個別のinit.lisp経由でQEMU上で直接評価し、`before-panic`の出力後に新設の診断メッセージでpanicし、以降の`after-panic-should-not-print`/`TEST-DONE`が出力されないことを確認した。`make build`が警告無く成功し、`make test`で既存の全19フィクスチャに回帰が無いことを確認した。 |
| 67 | フェーズ3統合検証 | 完了 | `make build`が警告無く成功することを確認した。`make test`で`test/lisp/`配下の全19フィクスチャが回帰無くPASSすることを確認した。さらにmilestone59/62と同じ手法（QEMU/OVMF上のREPLへシリアルソケット経由で対話的に文字列を送り込む）で、milestone65の起動シーケンス変更後の実際の起動列（`compiler.lisp`をフラグ`false`のままロード→`mark-compiler-ready`→`stdlib.lisp`を新経路でロード→REPL）を経て`(compiler-ready-p)`→`t`が返ることを確認し、続けて`compiler.lisp`へ移設した6関数（`(mapcar (lambda (x) (+ x 1)) (list 1 2 3))`→`(2 3 4)`・`(append (list 1 2) (list 3 4))`→`(1 2 3 4)`・`(reverse (list 1 2 3))`→`(3 2 1)`・`(not nil)`→`t`）と、`stdlib.lisp`側の関数（`(reduce + (list 1 2 3 4) 0)`→`10`・`(sort < (list 3 1 4 1 5))`→`(1 1 3 4 5)`）、`defun`のコンパイル時コード生成と呼び出しディスパッチ（`(defun sq (x) (+ x x))`→`sq`・`(sq 21)`→`42`・`(funcall sq 6)`→`12`・`(apply sq (list 7))`→`14`）が単一のQEMUセッション内で連続して正しく評価されることを確認した。検証中`(sq 7)`相当の乗算呼び出し（`(defun sq (x) (* x x))`→`(sq 7)`）が`unbound variable`でpanicすることを再確認したが、これはmilestone62で既に文書化した既知のスコープ外事項（乗算ビルトイン`*`が未実装）の再現であり、本ロードマップの回帰ではない。むしろこのpanicがREPLのlongjmp復帰トラップ（milestone31）で正しく捕捉され、そのまま次の入力プロンプトへ戻ったこと自体が、milestone65の起動シーケンス変更後もpanic復帰機構に回帰が無いことの確認になった。以上により、フェーズ3（milestone63・65・66）完了後もビルド・全フィクスチャ・起動シーケンス全体（`compiler.lisp`→`stdlib.lisp`→REPL）に回帰が無いことを実地で確認し、本ロードマップ（milestone48〜67）の全項目が完了した。 |
