...
| 128 | 境界処理の統合 | 完了 | `src/main.c`の起動シーケンス冒頭で直接呼んでいた`ConOut->ClearScreen`を`lisp_screen_buffer_init()`呼び出しに置き換えた(QueryModeでcols/rows確定・実ClearScreen・back/front初期化を1回で済ませる。Lisp heapへは触れないため`lisp_heap_init`より前のこの位置でも安全、という制約はmilestone127のバグ修正で確立済みの知見と同じ)。REPLループのプロンプト`"> "`と評価結果の後ろの改行を、直接`ConOut->OutputString`していた実装から`lisp_print_ascii(&console_stream, ...)`(バッファ経由)へ切り替え、その直後に明示的`lisp_screen_flush()`を追加した(プロンプトは`lisp_read_line`のブロッキング待ちに入る前に、結果表示は次のプロンプトが出る前に、それぞれ確実に画面へ反映されるようにするため)。`lisp_print_ascii`をLisp heap非依存の生ASCII出力ヘルパーとして`src/lisp.h`へ新規公開した。`lisp_panic`/`lisp_panic_fatal`(`src/lisp.c`)の冒頭に`lisp_screen_flush()`を追加し、直接ConOutへ書くpanicメッセージの前に画面バッファの未flush分(ツリーウォーク経路(`lisp_eval`/`lisp_apply`)で書かれた内容等、milestone127で明記した既知の制約に該当するもの)を先に反映することで、表示順序が崩れないようにした(未初期化状態でも`lisp_screen_flush`自身がdirty/pending_newlinesとも0のため何もせず即座に戻る安全設計)。キー入力エコー(`lisp_read_line`内の直接`ConOut->OutputString`)は対話性優先で意図的にバッファ対象外のまま変更しなかった。`make build`/`make test`(29ファイル全PASS)で確認した。フルスイート1回目で`test-package`が`TIMEOUT`したが、単体`make test-package`では即座にPASSし、変更内容(main.c/lisp.cへのpure additive変更)とは無関係な既知の非決定的QEMUテストハーネス事象(milestone33以来記録されている類のもの)と判断し、フルスイート再実行で29ファイル全PASSを再確認した。 |
...
### 続報: milestone127自己テストの検証用'X'が実画面に残留するバグ(process_screen_switch milestone138後に発覚)

`documents/lisp_process_screen_switch.md`milestone138(Ctrl単体押下時に画面左上へ'C'を
表示する機能)を実機で使用したユーザーから「Ctrlを押してもXのままになっている」という報告が
あった。調査の結果、milestone138の'C'表示自体(行0・列0)は無関係で、原因は
milestone127の自己テスト`lisp_vm_flush_hook_selftest`(`src/lisp.c`)が、VMディスパッチ
ループのflushフックを検証するために書き込む`'X'`文字だった。この文字は`front[LISP_SCREEN_
STATUS_ROWS][0]`(行1・列0、'C'表示のちょうど1行下)へ実際に`lisp_screen_flush`経由で
実画面へ描画されるが、検証完了後に消去する処理が無く、その後の起動シーケンスもREPL開始も
このセルを一度も上書きしないため、実画面上に恒久的に残り続けていた。

`lisp_vm_flush_hook_selftest`の検証(`front[...] == 'X'`確認)が終わった直後に、
`lisp_screen_flush`内部の既存のリトライ付き直接書き込みヘルパー(`lisp_screen_flush_set_
cursor_position`/`lisp_screen_flush_output_string`、milestone133由来)を使って同じ座標へ
半角スペースを直接上書きし、続けて`lisp_screen_buffer_init()`でback/front/dirty/touchedの
ソフトウェア側状態もこの直接書き込みに合わせて再初期化した。`lisp_screen_buffer_init()`
単体ではソフトウェア側の記録のみが空白に戻り実画面の'X'自体は消えないため、実画面への
直接消去と内部状態のリセットの両方が必要だった点が実装時の要点。

`make build`および`make test`を2回実行し、いずれも既知のbug#3クラス(`Lisp panic:
lisp_screen_flush: SetCursorPosition failed`、固定の起動時自己テスト列の中で本修正とは
無関係な位置で発生)以外の回帰が無いことを確認した。ヘッドレスのシリアルログはANSI/VT100
エスケープを除去した生バイト列であり実際の2次元画面上での視覚的な上書きは検証できないため
(milestone118以来の既知の制約と同種)、実機/QEMU GUIでの目視確認はユーザー側に委ねる。
