...
| 33 | マーク＆スイープGCの実装と固定スロット再利用 | ✅ 完了 | milestone32の全オブジェクト追跡を土台に、実際にマーク＆スイープを実行するGCを実装した。**固定スロット再利用:** `gc_next`（オフセット0、milestone32で追加済み）を「全オブジェクト追跡リストの次ポインタ」と「フリーリストの次ポインタ」の両方に流用し、タグ（`LISP_TAG_CONS`/`LISP_TAG_SYMBOL`/`LISP_TAG_CLOSURE`）ごとに3本のフリーリスト（`lisp_free_cons`/`lisp_free_symbol`/`lisp_free_closure`）を新設。`lisp_alloc_tracked`は対応するフリーリストが空でない場合そこから再利用し、空の場合のみ従来通り`lisp_alloc`でバンプ確保する。**マークフェーズ（`lisp_gc_mark`）:** `rplacd`（milestone27）で作れる自己参照circularリストに対して無限再帰・無限ループしないよう、各オブジェクトは「未マークなら`gc_marked`を立ててから」子を辿る規約を徹底。`cdr`・シンボルの`value`・closureの`env`方向は確保数に比例してCスタックが伸びるのを避けるため再帰せず`while(1)`ループで辿り、`car`・`params`・`body`・vectorの各要素方向のみ再帰する（幅方向は現実的なプログラムでは深くならない一方、`cdr`鎖は長大なリストで数十万要素に達し得るため）。**ルート集合（`lisp_gc_mark_roots`）:** `global_env`、各パッケージテーブルに登録された全intern済みシンボル、`lisp_return_tag`/`lisp_return_value`（milestone31由来の既知バグにより`return-from`のタグ不一致panic時にクリアされないため、GCとしては保守的に常に生存扱いとする）。`t`等の特殊シンボルキャッシュは全て`lisp_intern`経由でパッケージに登録済みのため、パッケージ走査で既に到達可能であり個別マークは行わない（冗長コードを避けた）。**スイープフェーズ（`lisp_gc_sweep`）:** `lisp_all_objects_head`を1回走査し、マーク済みは生存リストへ再連結（マークビットをリセット）、未マークはタグに応じたフリーリストへ再連結して回収数を返す。**しきい値による自動発動:** `lisp_heap_low()`がバンプ側残り容量がヒープ総量の20%未満かを判定し、`src/main.c`のREPL`for(;;)`ループで`lisp_setjmp(&repl_trap)`直後・プロンプト表示前（milestone31が確立した安全地点）に`lisp_gc()`を呼ぶ。評価中は一切GCを起動しない（正確なスタックスキャンをスコープ外とした制約から直接導かれる仕様）。**手動発火:** `(gc)`組み込み関数（`lisp_builtin_gc`）を追加し、回収オブジェクト数をfixnumで返す。**既知の制限（対応せず明記）:** スイープで回収された`LispClosure`が文字列/bignum/vectorだった場合、その2次バッファ（`str_data`/`big_digits`/`vec_data`）は解放されず永久リークになる——固定スロットの再利用のみを対象としコンパクションは行わないという既存のスコープ外方針と同じ精神で、本milestoneでは対応しない。検証はQEMU/OVMFヘッドレス起動で、milestone30〜32の既存回帰項目に加え、循環リストへの`(gc)`呼び出しがハングしないこと（cycle-safety）、`churn`関数で大量のゴミを生成した後の`(gc)`が0より大きい回収数を返すこと、ヒープ総量の80%超を消費してもしきい値越えで自動的にGCが走り`heap exhausted`にならず処理が継続すること、`test/lisp/`配下の既存12フィクスチャ全てが単独実行・連続実行いずれでも`t`を返すことを確認した。検証中に一度だけ、複数のQEMUインスタンスを並行起動していた状況下で`(run-test-vector)`呼び出し直後にゲストが予期せずリセットする事象を観測したが、同一フィクスチャ列を単独QEMUインスタンスで3回再現試行した限りでは再現せず（`(run-test-vector)`を連続6回呼んでも`t`が続き、その後の`(gc)`も正常動作）、また未初期化フィールド・ヒープ枯渇による暗黙の破壊・入力バッファオーバーフローの3つの仮説はいずれもソースコードの直接確認により否定された。ホスト側のリソース競合等によるテストハーネス起因の事象である可能性が高いと判断し、既知の未解決事項として記録するに留めた |
