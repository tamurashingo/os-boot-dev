...
(defun run-test-assembler-literal-operand ()
  ; オペランドがfixnumならそのまま2byte(リトルエンディアン)へ分割してバイト値として使う
  (struct-eq (assemble (list (list *op-const* 5) (list *op-return*)))
             (list *op-const* 5 0 *op-return*)))

(defun run-test-assembler-forward-jump ()
  ; まだ出現していないラベルへの前方参照も解決できる。
  ; (op-jump (ref end)) [3byte] -> (op-const 5) [3byte] -> (label end) -> (op-return) [1byte]
  ; endの絶対位置は6
  (struct-eq (assemble (list (list *op-jump* (list 'ref 'end))
                              (list *op-const* 5)
                              (list 'label 'end)
                              (list *op-return*)))
             (list *op-jump* 6 0 *op-const* 5 0 *op-return*)))
...
(defun run-test-assembler-loop-shape ()
  ...
  (struct-eq
   (assemble (list (list 'label 'loop)
                    ...
                    (list 'label 'end)
                    (list *op-return*)))
   (list *op-const* 1 0 *op-jump-if-false* 9 0 *op-jump* 0 0 *op-return*)))
